...
# -----------------------------------------------------------------------------
# TRAINED FREE-DATA ENSEMBLE (presentation stays in this legacy dashboard)
# -----------------------------------------------------------------------------
@st.cache_resource
def get_free_ensemble_services():
    """Create one shared retrying, cached, rate-limited data client."""
    settings = load_free_settings()
    http = FreeHttpClient(
        settings.cache_path,
...
def _free_game_ref(game):
    commence = datetime.fromisoformat(game['game_datetime_utc'].replace('Z', '+00:00'))
    return FreeGameRef(
        game_pk=int(game['game_pk']),
        game_date=game.get('game_date') or commence.date().isoformat(),
...
def run_free_ensemble(game):
    ...
    raw_game = {
        'teams': {
            'away': {'probablePitcher': {'id': game.get('away_pitcher_id'), 'fullName': game.get('away_pitcher')}},
            'home': {'probablePitcher': {'id': game.get('home_pitcher_id'), 'fullName': game.get('home_pitcher')}},
        }
    }
    coordinates = {}
    if feed.usable:
        coordinates = (feed.payload.get('gameData', {}).get('venue', {})
                       .get('location', {}).get('defaultCoordinates', {})) or {}
    weather = weather_client.forecast(
        coordinates.get('latitude'), coordinates.get('longitude'), ref.commence_time
    )

    feature_error = ""
    try:
        snapshot, coverage = FreeLiveFeatureBuilder(mlb, bundle.metadata).build(
            ref, raw_game, feed, weather
        )
    except Exception as exc:
        # A temporary provider failure must not blank the whole slate. Keep
        # the historical Elo feature and let the fitted pipelines use their
        # native missing-value handling for the rest, visibly labeled below.
        values = {name: float('nan') for name in bundle.feature_names}
        ratings = bundle.metadata.get('elo_ratings', {})
        home_code, away_code = TEAM_CODES.get(ref.home_team), TEAM_CODES.get(ref.away_team)
        if 'team_elo_rating_diff' in values:
            values['team_elo_rating_diff'] = (
                float(ratings.get(home_code, 1500)) - float(ratings.get(away_code, 1500))
            )
        snapshot = FreeFeatureSnapshot(
            ref, values, {'team_elo_rating_diff': 'Retrosheet historical Elo'}
        )
        coverage = sum(math.isfinite(float(v)) for v in values.values()) / max(1, len(values))
        feature_error = str(exc)
        stage = f"{stage} (limited live data)"
...
def model_moneyline_game(game, market_map, min_edge, min_prob, strict_lineup_mode, season, standings_map, ops_leaders_map):
    ...
    # Show a projection at every stage. Strict mode now controls whether an
    # early projection can be called a qualified wager; it no longer hides
    # the projection itself.
    is_ready = ensemble_result is not None
    ...
    if has_ml_market:
        ...
        is_qualified = recommendation_ready and (edge >= min_edge) and (model_prob >= max(min_prob, 0.58))
    ...
    return {
        ...
        'backend_model': bool(ensemble_result),
...
def apply_calibration_correction(r, calibration_lookup, min_edge, min_prob, min_sample=30):
    ...
    # The free-data artifact is already fitted with Platt calibration on a
    # held-out validation set. Never overwrite it with the legacy database's
    # coarse probability buckets.
    if r.get('backend_model'):
        r['artifact_calibrated'] = True
        ...
        if r.get('has_ml_market') and r.get('implied_prob') is not None:
            ...
            r['is_qualified'] = r.get('recommendation_ready', False) and (r['edge'] >= min_edge) and (r['model_prob'] >= max(min_prob, 0.58))
        return r
...
with st.spinner(f"Loading real-time stats for {len(games)} game(s) on {date_str}... (fetching team/pitcher data in parallel)"):
    ...
    # latency keeps a full slate from taking one-game-at-a-time minutes.
        ...
        with ThreadPoolExecutor(max_workers=min(20, len(games))) as _ensemble_executor:
            ...
            for _future in as_completed(_ensemble_futures):
                try:
                    ...
                    pass  # the individual game displays a transparent unavailable state below
...
for idx, r in enumerate(non_final_results, 1):
    ...
    elif not r.get('recommendation_ready', True):
        status_tag = "🟡 Prelim"
        status_tag_full = "🟡 PRELIMINARY — NOT BET-READY"
    ...
    ml_table_data_full.append({
        ...
        "Calibrated?": (
            "✅ Platt (held-out data)" if r.get('artifact_calibrated') else
...
st.caption(
    ...
    "in strict mode. The trained ensemble probability is Platt-calibrated on held-out historical games."
)
